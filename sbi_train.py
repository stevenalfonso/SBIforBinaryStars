...
mass_bounds = [0.08, 6.0]           # M1
metallicity_bounds = [-2.0, 0.6]    # [Fe/H]
distance_bounds = [10.0, 200.0]     # distance
...
if selec_prior == 1:
    ...
    prior = [
    TransformedDistribution(Exponential(rate=tt([1.35])), \
...
# #df = df[(df['bp_rp'] > -0.5) & (df['bp_rp'] < 3.)]
# df = df[(df['flag'] == 1)]
# #df['g_abs'] = df['RPmag'] - 5*np.log10(1000*df['Dist50']) + 5
# df = df[df['[Fe/H]'].notna()]

# cond_metallicity = (df['[Fe/H]'] > metallicity_bounds[0]) & (df['[Fe/H]'] < metallicity_bounds[1])
# cond_mass = (df['mass'] > mass_bounds[0]) & (df['mass'] < mass_bounds[1])
# data_binary = df[cond_metallicity & cond_mass]
